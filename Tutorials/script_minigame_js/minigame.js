...
function draw_crosshair() {
    ...
    this.canvas.draw_line(screen_center.x + 10, screen_center.y, (screen_center.x + 30), (screen_center.y), {
        r: 255,
        g: 255,
        b: 255,
        a: 255
    ...
    this.canvas.draw_line(screen_center.x, screen_center.y + 10, (screen_center.x), (screen_center.y + 30), {
        r: 255,
        g: 255,
        b: 255,
        a: 255
...
function create_explosion(pos, obj_hit) {
    ...
    this.explosions.create_solid_particles(pos, {
        x: 0,
        y: 0,
        z: 1
    }, emit_radius, 0.04 * Math.max(1.0, Math.log2(emit_radius)), 10, 1);
...
function spawn_invader_line(invader_path, SPECIAL_INVADER_PATH = NORMAL_INVADER_PATH, num_of_stronger_invaders = 0) {
    ...
    let start_loc = {
        x: -1942405.2868467504,
        y: 5212454.450893189,
        z: 3122019.4702613624
    };
    let start_rot = {
        x: 0.22153694927692413,
        y: 0.4521426856517792,
        z: 0.6030480861663818,
        w: -0.618725597858429
    };
    ...
    if (invader_array.length == 0) {
        start_loc = {
            x: -1941927.023828117,
            y: 5212336.678825753,
            z: 3122498.8268912034
        };
        start_rot = {
            x: 0.04354477673768997,
            y: 0.5016129612922668,
            z: 0.7856196165084839,
            w: -0.3595695197582245
        };
    } else if (invader_array.length == 1) {
        start_loc = {
            x: -1942146.710236884,
            y: 5212422.649997227,
            z: 3122225.6552367667
        };
        start_rot = {
            x: 0.04354477673768997,
            y: 0.5016129612922668,
        ...
        start_loc = {
            x: -1942171.8067069917,
            y: 5212432.471147353,
            z: 3122194.4487372655
        };
        start_rot = {
            x: 0.04354477673768997,
            y: 0.5016129612922668,
    ...
    for (let i = 0; i < invaders_in_line; i++) {
        ...
        if (stronger_invaders.includes(i)) {
            ...
            // 4. param - if object is persistent(0), colliding(1) or kinematic(2)
            //            0 - none
...
//Remove remaining spawned invaders from the scene
...
// Reload game state
// This function is called from "Game over" HTML menu, when "Reload" is pressed
    ...
    if (mouse_button == 0) {
        ...
        if (projectiles_loaded >= 1) {
            ...
            if (cannon) {
                ...
                // returns - trace id
                this.explosions.launch_tracer(barrel_tip_pos_ECEF, camera_direction, 100, {
                    x: 0.5,
                    y: 0.5,
                    z: 0.5,
                    w: 1
...
// Event called, before script is reloaded
function on_before_reload() {
    // Note: on script reload, the game state is automatically reloaded,
    // but the spawned objects (cannon and invaders) need to be correctly removed from scene before that happens.
...
function on_initialize() {
    ...
    // 3. param - radius
    this.create_sensor({
        x: -1941638.9198791014,
        y: 5211794.163754703,
        z: 3123587.0791307855
    }, {
        x: 0,
        y: 0,
        z: 0,
...
function before_simulation_step(dtns, ns_sim) {
    ...
    if (cannon_loaded && cannon_entered === false) {
        ...
        if (cannon == null) {
            // Spawn cannon
            let cannon_start_pos = {
                x: -1941609.2055510941,
                y: 5212173.27245129,
                z: 3123161.9673035163
            };
            let cannon_start_rot = {
                x: -0.49617794156074524,
                y: -0.09487587213516235,
                z: 0.14400778710842133,
                w: 0.8509216904640198
            };
...
function visual_update(dtrender, dtsim, dtinterpolate) {
    ...
    if (!is_loc_ready || !cannon || !cannon_geom || !cannon_entered || !invader_01_loaded || !invader_02_loaded) {
        ...
        this.canvas.fill_rect(0, 0, screen_size.x, screen_size.y, {
            r: 0,
            g: 0,
            b: 0,
            a: 255
        ...
        // 4. param - text
        // 5. param - color
        this.canvas.draw_text(font, screen_center.x - 70, screen_center.y - 7, "Loading minigame... ", {
            r: 255,
            g: 255,
            b: 255,
            a: 255
    ...
    this.canvas.fill_rect(20, screen_size.y - 20, 210 + 40 * score.toString().length, -50, {
        r: 15,
        g: 0,
        b: 60,
        a: 255
    ...
    this.canvas.draw_text(font_xolonium_50_bold, 30, screen_size.y - 67, "score: " + score, {
        r: 0,
        g: 150,
        b: 150,
        a: 255
    ...
    this.canvas.draw_text(font, screen_size.x - 250, screen_size.y - 67, "Invaders remaining: " + instances_count, {
        r: 0,
        g: 0,
        b: 0,
        a: 255
    ...
    for (let p = 0; p < MAX_PROJECTILES; p++) {
        this.canvas.draw_image_wh(projectile_img, 30, 20 + (40 * p), 110, 30, {
            r: 255,
            g: p < projectiles_loaded ? 255 : 0,
            b: p < projectiles_loaded ? 255 : 0,
            a: 255
        });
    }
    ...
    for (let h = 0; h < MAX_HEARTS; h++) {
        this.canvas.draw_image_wh(heart_img, screen_size.x - 50 - (40 * h), 20, 40, 40, {
            r: 255,
            g: 255,
            b: 255,
            a: h < hearts_remaining ? 255 : 100
        });
    }
}

// Invoked after the final camera position in the frame is known.
// Cannon rotation is based on camera rotation, therefore the final camera position needs to be known, before setting cannon rotation.
...
function on_final_camera(pos, rot) {
    ...
    // Rotate cannon
    cannon_geom.rotate_joint_orig(cannon_pitch_id, angle, {
        x: axis.x,
        y: axis.y,
        z: axis.z
    });
