...
int bones::bones::FL_wheel = -1;
int bones::bones::FR_wheel = -1;
int bones::bones::RL_wheel = -1;
int bones::bones::RR_wheel = -1;
...
void tutorial_car_plugin::engine(int flags, uint code, uint channel, int handler_id)
{
	...
	//Get camera mode using get_camera_mode  
	//returns 0, when the first person view, inside the plane is active

	//Declare variable, that will be used to set sound gain, depending on, where the player camera is located
	...
	//Declare variable, that will be used to set reference distance, depending on, where the player camera is located
	//Based on the camera mode, set the reference distance
