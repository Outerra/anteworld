...
int bones::bones::FL_wheel;
int bones::bones::FR_wheel;
int bones::bones::RL_wheel;
int bones::bones::RR_wheel;
int bones::steer_wheel;
int bones::speed_gauge;
int bones::accel_pedal;
int bones::brake_pedal;
int bones::driver_door;
uint lights_entity::rev_mask;
uint lights_entity::brake_mask;
uint lights_entity::turn_left_mask;
uint lights_entity::turn_right_mask;
uint lights_entity::main_light_offset;
int sound_entity::snd_starter;
int sound_entity::snd_eng_running;
int sound_entity::snd_eng_OFF;
int sound_entity::src_ON_OFF;
int sound_entity::src_eng_running;
...
namespace 
{
	...
	IFC_REGISTER_CLIENT(tutorial_car);
}
...
void tutorial_car::engine(int flags, uint code, uint channel, int handler_id)
{
	if (!this->sounds)
	{
		return;
	...
	if (this->started == true)
	{
		...
		this->sounds->play_sound(sound_entity::src_ON_OFF, sound_entity::snd_starter);
	}
	else
	{
		//stop() discards all sounds playing on given emitter
		this->sounds->stop(sound_entity::src_eng_running);
		this->sounds->play_sound(sound_entity::src_ON_OFF, sound_entity::snd_eng_OFF);
	}
}
//Used to change direction and turn on/off reverse lights
void tutorial_car::reverse(int flags, uint code, uint channel, int handler_id)
{
...
void tutorial_car::emergency_lights(int flags, uint code, uint channel, int handler_id)
{
	//Bitwise operator XOR (^) used, to switch the value each time, this function is called.
	this->emer ^= 1;
}
//Used to turn on/off passing lights
void tutorial_car::passing_lights(float val, uint code, uint channel, int handler_id)
{
...
void tutorial_car::main_lights(float val, uint code, uint channel, int handler_id)
{
...
void tutorial_car::turn_lights(float val, uint code, uint channel, int handler_id)
{
	if (val == 0)
	{
...
void tutorial_car::open_door(float val, uint code, uint channel, int handler_id)
{
...
ot::chassis_params tutorial_car::init_chassis(const coid::charstr& params)
{
	//Define wheel setup
	...
	bones::FL_wheel = add_wheel("wheel_l0", wheel_params);
	bones::FR_wheel = add_wheel("wheel_r0", wheel_params);
	bones::RL_wheel = add_wheel("wheel_l1", wheel_params);
	bones::RR_wheel = add_wheel("wheel_r1", wheel_params);
	...
	// returns sound ID
	sound_entity::snd_starter = load_sound("sounds/starter.ogg");
	sound_entity::snd_eng_running = load_sound("sounds/2714_on.ogg");
	sound_entity::snd_eng_OFF = load_sound("sounds/2714_off.ogg");
	...
	sound_entity::src_ON_OFF = add_sound_emitter("exhaust_0_end");
	sound_entity::src_eng_running = add_sound_emitter("exhaust_0_end");
	...
	register_event_handler("vehicle/engine/on", &tutorial_car::engine);
	register_event_handler("vehicle/engine/reverse", &tutorial_car::reverse);
	register_event_handler("vehicle/lights/emergency", &tutorial_car::emergency_lights);
	...
	// returns slot id or -1 on fail
	register_axis_handler("vehicle/lights/passing", &tutorial_car::passing_lights, { .minval = 0.f, .maxval = 1.f, .cenvel = 0.f, .vel = 10.f });
	register_axis_handler("vehicle/lights/main", &tutorial_car::main_lights, { .minval = 0.f, .maxval = 1.f, .cenvel = 0.f, .vel = 10.f });
	register_axis_handler("vehicle/lights/turn", &tutorial_car::turn_lights, { .minval = -1.f, .maxval = 1.f, .cenvel = 0.f, .vel = 10.f });
	register_axis_handler("vehicle/controls/open", &tutorial_car::open_door, { .minval = 0.f, .maxval = 1.f, .cenvel = 0.f, .vel = 0.6f });
...
void tutorial_car::init_vehicle(bool reload)
{
...
void tutorial_car::update_frame(float dt, float engine, float brake, float steering, float parking)
{	
	//Check, if the Geom pointer is not nullptr before attempting to use it, to avoid crashing due to a null pointer
	...
	if (this->started == true)
	{
		...
		if (this->sounds->is_playing(sound_entity::src_ON_OFF))
		{
			engine = 0;
		}
		else if (this->started == true && !this->sounds->is_looping(sound_entity::src_eng_running))
		{
			//play_loop() is used to play sound in loop, breaking other sounds
			// 1.param - emitter (source ID)
			// 2.param - sound (sound ID))
			this->sounds->play_loop(sound_entity::src_eng_running, sound_entity::snd_eng_running);
		}
	}
	...
	wheel_force(bones::FL_wheel, engine);
	wheel_force(bones::FR_wheel, engine);
	...
	steer(bones::FL_wheel, steering);
	steer(bones::FR_wheel, steering);
	...
	if (this->started == true)
	{
		...
		this->sounds->set_pitch(sound_entity::src_eng_running, (0.5f * pitch) + 1.f);
		//Use set_gain() to set gain value on given emitter
		// 1.param - emitter 
		// 2.param - gain value (this will affect all sounds emitted from this emitter)
		this->sounds->set_gain(sound_entity::src_eng_running, (0.25f * pitch) + 0.5f);
	}
