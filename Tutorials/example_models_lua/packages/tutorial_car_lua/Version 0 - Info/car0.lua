...
-- Welcome to the first tutorial on Lua programming in Outerra.
-- We recommend reviewing the Outerra/anteworld tutorial on Github ( https://github.com/Outerra/anteworld/wiki/Tutorial-1-%E2%80%90-car-(Lua) ) before proceeding, as it provides detailed information.
...
-- It is needed to implement interface, to be able to use interface functionality (in this case ot.vehicle_script), such as init_chassis, init_vehicle, update_frame etc.
...
function ot.vehicle_script:init_chassis(param) 
	...
	-- Examples: self:register_event("air/controls/eject", function(v){...});
	--			 self:register_button("air/controls/eject", function(v){...});
	...
	-- Example:	self:register_axis("air/heli/collective", options, function(v,dt){...});
	...
	--	action - path to input binding (example: vehicle/lights/cabin) where:
    --              "vehicle" is the name of .cfg file, in which we have the binding defined (these can be found in bin/defaults/iomap directory, where vehicle.cfg is for vehicles, air.cfg is for aircraft etc.)
    --              "lights" is name of the group, to which it belongs
    --              "cabin" is the name of the bound action (this action is bound to shift+L))
    ...
    --              This function can have 3 parameters:	
	...
	self:register_event("vehicle/controls/brake", brake_action);
...
-- "self" is used to refer to the current instance of an object, in which the code is being executed
-- "self:" (with colon) is used for methods that pass self as the first parameter - "self:some_function()" is the same as "self.some_function(self)"
-- "self." (with period) is used for everything else.
...
-- If you want to call "self" in function, to refer to the current instance of an object, you have to pass it as parameter
