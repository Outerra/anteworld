...
//You should have at least init_chassis(), init_vehicle() and update_frame() functions

//init_chassis() is invoked only when the model is first time loaded, or upon reload, this is used to define the vehicle construction, binding bones(steering wheel, gauges, etc.), adding wheels, lights, sounds, sound emitters and action handlers, etc.. These things can be made only in init_chassis()
...
{ 
	...
	//	handler - function, which is invoked whenever given event occurs or the tracked value changes as the result of the button state. This function can have 3 parameters:	"v" - the action value itself(integer for events or floating point for values)
	...
	//	positions - number of positions/steps between minval and maxval, for example light switch on train has 4 positions, his minval is 0 and maxval is 3, when coresponding input is pressed, the switch will jump between 0,1,2 and 3.  
	...
	this.register_event("vehicle/controls/brake", BrakeAction);
	...
	//								grip_angle - grip angular offset in degrees (default: 8)
	//								bone - steering wheel bone name override (default: "steering_wheel")
