...
function init_chassis() {
    ...
    //Add action handlers
    this.register_axis("air/lights/landing_lights", {
        minval: 0,
        maxval: 1,
        vel: 10,
        center: 0
    }, function (v) {
        this.light_mask(0x3, v > 0);
    });
    this.register_axis("air/lights/nav_lights", {
        minval: 0,
        maxval: 1,
        vel: 10,
        center: 0
    }, function (v) {
        this.light_mask(0x3, v > 0, nav_light_offset);
    });
    ...
    this.register_axis("air/controls/aileron", {
        minval: -1,
        maxval: 1,
        center: 0.5,
        vel: 0.5,
    ...
    this.register_axis("air/controls/elevator", {
        minval: -1,
        maxval: 1,
        center: 0.5,
        vel: 0.5,
    ...
    this.register_axis("air/controls/rudder", {
        minval: -1,
        maxval: 1,
        center: 0.5
        vel: 0.5
    ...
    this.register_axis("air/controls/brake", {
        minval: 0
    }, function (v) {
        this.braking = v;
...
function update_frame(dt) {
    ...
    this.geom.rotate_joint(bones.propeller, dt * (2 * PI) * (propeller_rpm), {
        y: 1
    });
    this.geom.rotate_joint(bones.wheel_front, dt * PI * (wheel_speed / 5), {
        x: -1
    });
    this.geom.rotate_joint(bones.wheel_right, dt * PI * (wheel_speed / 5), {
        x: -1
    });
    this.geom.rotate_joint(bones.wheel_left, dt * PI * (wheel_speed / 5), {
        x: -1
    });
    //rotate_joint_orig - rotate to given angle (from default orientation)
    this.geom.rotate_joint_orig(bones.elevator_right, elev_pos_rad, {
        x: 1
    });
    this.geom.rotate_joint_orig(bones.elevator_left, elev_pos_rad, {
        x: 1
    });
    this.geom.rotate_joint_orig(bones.rudder, this.jsbsim['fcs/rudder-pos-rad'], {
        z: -1
    });
    this.geom.rotate_joint_orig(bones.aileron_right, this.jsbsim['fcs/right-aileron-pos-rad'], {
        x: -1
    });
    this.geom.rotate_joint_orig(bones.aileron_left, this.jsbsim['fcs/left-aileron-pos-rad'], {
        x: 1
    });
    this.geom.rotate_joint_orig(bones.flap_left, this.jsbsim['fcs/flap-pos-rad'], {
        x: 1,
        y: 0,
        z: 0
    });
    this.geom.rotate_joint_orig(bones.flap_right, this.jsbsim['fcs/flap-pos-rad'], {
        x: 1,
        y: 0,
        z: 0
    });
    this.geom.rotate_joint_orig(bones.brake_pedal_left, this.jsbsim['fcs/left-brake-cmd-norm'] * 0.7, {
        x: -1
    });
    this.geom.rotate_joint_orig(bones.brake_pedal_right, this.jsbsim['fcs/right-brake-cmd-norm'] * 0.7, {
        x: -1
    });
    ...
    if (this.jsbsim['fcs/rudder-pos-rad'] > 0) {
        this.geom.move_joint_orig(bones.rudder_pedal_left, {
            y: (this.jsbsim['fcs/rudder-pos-rad'] / 5)
        });
    } else if (this.jsbsim['fcs/rudder-pos-rad'] < 0) {
        this.geom.move_joint_orig(bones.rudder_pedal_right, {
            y:  - (this.jsbsim['fcs/rudder-pos-rad'] / 5)
        });
    }

    //Move throttle handle, depending on the throttle command value
    this.geom.move_joint_orig(bones.throttle_handle, {
        y: (this.jsbsim['fcs/throttle-cmd-norm[0]'] / 7)
    });
