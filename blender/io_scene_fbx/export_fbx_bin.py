            ...
            if ob.type == 'MESH':
                ...
                for mod in ob.modifiers:
                    ...
                    if mod.show_render:
                        use_org_data = False
            if not use_org_data:
                tmp_me = ob.to_mesh(scene, apply_modifiers=True, settings='PREVIEW')
                for key in ob.data.keys():
                    tmp_me[key] = ob.data[key]
...
def fbx_header_elements(root, scene_data, time=None):
    ...
    if scene_data.settings.apply_unit_scale:
        ...
        scale_factor = scene_data.settings.global_scale * units_blender_to_fbx_factor(scene)
    elem_props_set(props, "p_integer", b"UpAxis", 2)
    elem_props_set(props, "p_integer", b"UpAxisSign", 1)
    elem_props_set(props, "p_integer", b"FrontAxis", 1)
    ...
    elem_props_set(props, "p_integer", b"OriginalUpAxis", 2)
