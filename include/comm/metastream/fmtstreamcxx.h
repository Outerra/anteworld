...
public:
    ...
    {
        //allow read-until-whitespace for strings
        int ign = allow_unquoted && t.type == type::T_CHAR ? -1 : 1;

        const lexer::lextoken& tk = _tokenizer.next(ign);
        token tok = tk;
        ...
        else
        {
            ...
            case type::T_CHAR:
            {
                if (!t.is_array_element())
                {
                    if (tk == lexign || tk == lexchr || tk == lexchre)
    ...
    virtual opcd read_array_content(binstream_container_base& c, uints n, uints* count, metastream* m) override
    {
        ...
        {
            if ((t.type != type::T_CHAR || tk != lexign) && tk != lexstr && tk != lexstre)
                return ersSYNTAX_ERROR;
        }
...
protected:
    ...
    bool separators_are_optional = false;
    bool allow_unquoted = false;
