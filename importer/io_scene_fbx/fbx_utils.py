...
class ObjectWrapper(metaclass=MetaObjectWrapper):
    ...
    * Blender DupliObject
    ...
    __slots__ = (
        ...
        '_tag', '_ref', '_dupli_matrix','parented_to_bone'
    )
    ...
    def __init__(self, bdata, armature=None):
        ...
        if isinstance(bdata, Object):
            ...
            self.bdata = bdata
            self._ref = None
            if bdata.parent_type == 'BONE':
                self.parented_to_bone = True
    ...
    def get_matrix_local(self):
        if self._tag == 'OB':
            if self.parented_to_bone:
                print(self.get_parent().matrix_global.inverted() * self.bdata.matrix_world.copy())
                return self.get_parent().matrix_global.inverted() * self.bdata.matrix_world.copy() 
            else:
    ...
    def fbx_object_matrix(self, scene_data, rest=False, local_space=False, global_space=False):
        ...
        print(self.name + " is in global space: " + str(is_global))
